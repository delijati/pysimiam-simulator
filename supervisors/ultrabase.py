...
class UltraBotSupervisor(Supervisor):

    """
    The UltraBotSupervisor inherits from the superclass 'supervisor.Supervisor'
    to implement detailed calculations for any inheriting UltraBot supervisor.
    Students are intended to inherit from this class when making their own
    supervisors.

    Most importantly, the UltraBotSupervisor object implements the system
    functions necessary to operate a UltraBot, namely the uni2diff unicycle to
    differential motion model conversion, the Jacobian problem, and any other
    computationally complex interface.
    ...
    implementation and is not required for students to properly implement their
    own supervisors."""
    ...
    def estimate_pose(self):
        ...
        x_new = x + x_dt
        y_new = y + y_dt
        pose = Pose(x_new, y_new, (theta_new + pi) % (2 * pi) - pi)
        return pose
    ...
    def ensure_w(self, v_lr):
        ...
        if v == 0:
            ...
            v_lim = max(
                min(abs(v), (R / 2) * (2 * v_max)), (R / 2) * (2 * v_min))
    ...
    def execute(self, robot_info, dt):
        ...
        ret = self.ensure_w(self.uni2diff(output))
        return ret
