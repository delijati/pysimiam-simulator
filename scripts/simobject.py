...
class SimObject(object):
    ...
    """The base class for all objects that can be drawn in the simulator.
       ...
       :param color: The internal color of the object (`0xAARRGGBB` or
                     `0xRRGGBB`). The default color is black.
       :type color: int
       """
...
class Cloud(SimObject):
    ...
    def add_point(self, pose):
        """
        Append a point at *pose* to the collection. The orientation of the pose
        is ignored
        """
        self.points.append((pose.x, pose.y))

    def draw(self, r, thickness=0):
        """
        Draw a polyline with modes at all added points, using the internal
        color
        """
        r.set_pose(self.get_pose())  # Reset everything
        r.set_pen(self.get_color(), thickness=thickness)
