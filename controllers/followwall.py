...
class FollowWall(PIDController):
    ...
    def get_heading(self, state):
        ...
        self.to_wall_vector = numpy.array(
            [(p_back.y - p_front.y) * ms / ds, (p_front.x - p_back.x) * ms / ds, 1])
        ...
        if offset > 0:
            return 0.3 * self.along_wall_vector + \
                2 * offset * self.to_wall_vector
        else:
            return 0.3 * self.along_wall_vector + \
                3 * offset * self.to_wall_vector
