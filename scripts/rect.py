...
class Rect(object):
        ...
        if isinstance(xywh, Rect):
            ...
            if len(xywh) == 4:
                self.left, self.bottom, self.width, self.height = (
                    float(i) for i in xywh)
            elif len(xywh) == 2:
                self.left, self.bottom, self.width, self.height = (
                    0.0, 0.0) + tuple(float(i) for i in xywh)
    ...
    def get_center(self):
        return self.left + \
            (self.width * 0.5), self.bottom + (self.height * 0.5)
    ...
    def collidepoint(self, xy):
        ...
        """
        x, y = xy
        return x >= self.left and x <= (
            self.left +
            self.width) and y >= self.bottom and y <= (
            self.bottom +
            self.height)
