...
class Simulator(threading.Thread):
    ...
    The simulator runs in a separate thread. None of its functions are
    thread-safe, and should never be called directly from other objects (except
    for the functions inherited from `threading.Thread`). The communication
    with the simulator should be done through its *in_queue* and *out_queue*.
    See :ref:`ui-sim-queue`.

    :param renderer: The renderer that will be used to draw the world.  The
    simulator will assume control of the renderer.  The renderer functions also
    have to be considered thread-unsafe.  :type renderer:
    :class:`~renderer.Renderer` :param in_queue: The queue that is used to send
    events to the simulator.  :type in_queue: :class:`Queue.Queue`
    """
    ...
    def read_config(self, filename):
        ...
        try:
            ...
            raise Exception(
                '[Simulator.read_config] Failed to parse ' +
                filename +
                ': ' +
                str(e))
        else:
    ...
    def __construct_world(self):
        ...
        for thing in self.__world:
            if thing.type == 'robot':
                try:
                    # Create robot
                    robot_class = helpers.load_by_name(
                        thing.robot.type, 'robots')
                    ...
                    # Create supervisor
                    sup_class = helpers.load_by_name(
                        thing.supervisor.type, 'supervisors')
            ...
            try:
                ...
                if self.__state == RUN or \
                   self.__state == RUN_ONCE:
                    ...
                    for i, sup in enumerate(self.__supervisors):
                        info = self.__robots[i].get_info()
                        inputs = sup.execute(info, time_constant)
                        self.__robots[i].set_inputs(inputs, sup)
                        self.fwd_logqueue()
    ...
    def __draw(self):
        ...
        if self.__robots and self.__center_on_robot:
            ...
            if self.__orient_on_robot:
                ...
                self.__renderer.set_screen_center_pose(
                    pose.Pose(robot.get_pose().x, robot.get_pose().y, 0.0))

        self.__renderer.clear_screen()

        if self.__draw_supervisors:
            for sup in self.__supervisors:
                sup.draw_background(self.__renderer)
        ...
        if self.__draw_supervisors:
            for sup in self.__supervisors:
                sup.draw_foreground(self.__renderer)
    ...
    def focus_on_world(self):
        ...
        def bloat_bounds(bounds, factor):
            ...
            h = yt - yb
            factor = (factor - 1) / 2.0
            return xl - w * factor, yb - h * \
                factor, xr + w * factor, yt + h * factor

        self.__center_on_robot = False
        bounds = self.__robots[0].get_bounds()
        for robot in self.__robots:
            bounds = include_bounds(
                bounds, bloat_bounds(robot.get_bounds(), 4))
        for obstacle in self.__obstacles:
    ...
    def apply_parameters(self, robot, parameters):
        ...
        The parameters have to correspond to the requirements of the
        supervisor, as specified in
        :meth:`supervisor.Supervisor.get_ui_description` """
    ...
    def add_plotable(self, expression):
        """A plotable is an expression that yields a numerical value. It is
        evaluated every cycle and the values are announced by the simulator in
        the ``plot_update`` signal.
    ...
    def start_simulation(self):
        ...
        if self.__robots is not None:
            [x.resume() for x in self.__robots if hasattr(x, 'resume')]
            self.__state = RUN
    ...
    def pause_simulation(self):
        ...
        if self.__robots is not None:
            [x.pause() for x in self.__robots if hasattr(x, 'pause')]
        self.__state = PAUSE
    ...
    def reset_simulation(self):
        ...
        if self.__robots is not None:
            [x.reset() for x in self.__robots if hasattr(x, 'reset')]
        self.__state = DRAW_ONCE
        self.__reset_world()
