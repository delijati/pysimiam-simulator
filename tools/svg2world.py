...
path_subc = re.compile(
    '[MmZzLlHhVvCcSsQqTtAa]\s*(?:,?\s*[-+]?[0-9]*\.?[0-9]+(?:[eE][-+]?[1-9][0-9]*)?)*')
...
def parse_path(pstr):
    ...
    for subc in subcs:
        command = subc[0]
        numbers = [
            float(nstr) * size_multiplier for nstr in path_num.findall(subc)]
...
def add_element(parent, tag, style, points, transform):
    ...
    pose = ET.SubElement(
        element, 'pose', {'x': str(x0), 'y': str(y0), 'theta': "0"})
...
if __name__ == "__main__":
    ...
    for path in root.findall('path'):
        ...
        if len(points) < 3:  # Robot
            ...
            if color is not None:
                robot = ET.SubElement(
                    world_root, 'robot', {
                        'type': robot_class, 'color': color.group(1)})
            else:
                robot = ET.SubElement(
                    world_root, 'robot', {'type': robot_class})
            ...
            pose = ET.SubElement(
                robot, 'pose', {'x': str(x), 'y': str(y), 'theta': str(theta)})
            geom = ET.SubElement(
                robot, 'supervisor', {'type': robot_supervisor})
    ...
    with open(sys.argv[2], 'w') as f:
        dom.parseString(ET.tostring(world.getroot())).writexml(
            f, '', '    ', '\n')
