...
from scripts.xmlreader import XMLReader
from scripts.xmlwriter import XMLWriter
from collections import OrderedDict
from traceback import format_exception
import sys
from scripts.ui import uiParameter
    ...
    def __init__(self, label, parameters):
        ...
        for key, value in parameters:
            ...
            if isinstance(value, float):
                self.leafs[dict_key] = FloatEntry(
                    child_label, value, 1.0, -1000.0, 1000.0)
            # A bool is an int, so it is important to check for bools first
            elif isinstance(value, bool):
                self.leafs[dict_key] = BoolEntry(child_label, value)
            elif isinstance(value, int):
                self.leafs[dict_key] = IntEntry(child_label, value, -100, 100)
            elif isinstance(value, tuple):
                self.leafs[dict_key] = ChoiceEntry(
                    child_label, value[0], value[1])
            elif isinstance(value, uiParameter):
                if value.type == uiParameter.INT:
                    self.leafs[dict_key] = IntEntry(
                        child_label,
                        value.value,
                        value.min_value,
                        value.max_value)
                    ...
                    self.leafs[dict_key] = ChoiceEntry(
                        child_label, value.value, value.value_list)
                elif value.type == uiParameter.GROUP:
                    self.leafs[dict_key] = Group(child_label, value.contents)
                else:
                    raise ValueError(
                        "Unrecognized parameter type {}".format(value.type))
            else:
...
class ParamWidget(QtGui.QWidget):
    ...
    def save_click(self):
        ...
        if filename is not None:
            writer = XMLWriter(
                filename, 'parameters', self.contents.get_xmlstruct())
            try:
                writer.write()
