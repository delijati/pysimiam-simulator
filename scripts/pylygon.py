...
_unzip = lambda zipped: zip(*zipped)
...
class Polygon(object):
    ...
    def distance(self, other, r=array([0, 0])):
        ...
        while dot(v, v) - dot(w, v) > _MACHEPS:  # while w is closer to origin
            ...
            if len(support) == 3:
                return v  # the origin is inside W; intersection
            w = support.add(-v)
        return v
    ...
    def C(self):
        ...
        if n == 1:
            return P[0]
        if n == 2:
            return array([X[0] + X[1] / 2, Y[0] + Y[1] / 2])
